...
(function($){
	...
	$.fn.edslider = function(settings){
		...
		var defaults = {
			...
			navigator : true,
			progress  : true,
			loadImgSrc: 'static/edslider/images/load.gif',
			skin      : 'edslider'
		};
		...
		this.each(function(){
			...
			function play(){
				...
				current = sliderLi
					.filter('.current')
					.siblings()
					.fadeOut(options.animation && options.duration || 0,function(){
						$(this).find('img').animate({height:'100%',width:'100%'},"slow");
					})
					.end()
					.fadeIn(options.animation && options.duration || 0, function(){
						interval();						 
  						current.find('img').animate({height:options.scaleY,width:options.scaleX},options.interval);
